...
export default function DashboardPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              ...
              {!statsLoading && ((stats?.expiring_packages || 0) + (stats?.total_pending_approvals || 0) + pendingPkgCount) > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center animate-pulse">
                  {(stats?.expiring_packages || 0) + (stats?.total_pending_approvals || 0) + pendingPkgCount}
                </span>
              )}
            </button>
            {/* Dropdown */}
              ...
              <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                    {((stats?.expiring_packages || 0) + (stats?.total_pending_approvals || 0) + pendingPkgCount)} Active
                  </span>
                </div>
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                  {/* Pending Approvals Notifications */}
                  {(stats?.pending_service_bookings || 0) > 0 && (
                    <Link to="/admin/customer-bookings" onClick={() => setShowNotifications(false)} className="px-4 py-3 hover:bg-red-50/50 transition-colors block">
                      <p className="text-xs font-bold text-red-600 flex items-center gap-1.5">
                        <Clock size={13} /> {stats.pending_service_bookings} Service Booking(s) Pending
                      </p>
                      <p className="text-[10px] text-gray-500 mt-0.5">Click to review & approve slot bookings</p>
                    </Link>
                  )}
                  {(stats?.pending_package_requests > 0 || pendingPkgCount > 0) && (
                    <Link to="/admin/package-approvals" onClick={() => setShowNotifications(false)} className="px-4 py-3 hover:bg-red-50/50 transition-colors block">
                      <p className="text-xs font-bold text-red-600 flex items-center gap-1.5">
                        <Package size={13} /> {stats?.pending_package_requests || pendingPkgCount} Package Request(s) Pending
                      </p>
                      <p className="text-[10px] text-gray-500 mt-0.5">Click to review & approve customer packages</p>
                    </Link>
                  )}
                  {(stats?.pending_product_orders || 0) > 0 && (
                    <Link to="/admin/product-orders" onClick={() => setShowNotifications(false)} className="px-4 py-3 hover:bg-red-50/50 transition-colors block">
                      <p className="text-xs font-bold text-blue-600 flex items-center gap-1.5">
                        <Zap size={13} /> {stats.pending_product_orders} Product Order(s) Pending
                      </p>
                      <p className="text-[10px] text-gray-500 mt-0.5">Click to view product orders & payment verification</p>
                    </Link>
                  )}
                  ...
                  {((stats?.expiring_packages || 0) + (stats?.total_pending_approvals || 0) + pendingPkgCount) === 0 && (
      ...
      {/* ── Pending Customer Approvals Required Section ──────────────── */}
      {stats && ((stats.total_pending_approvals || 0) > 0 || pendingPkgCount > 0) && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-8 p-5 bg-gradient-to-r from-red-950 via-slate-900 to-amber-950 rounded-2xl border-2 border-red-500/40 shadow-xl text-white relative overflow-hidden"
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-black tracking-wide text-white">PENDING CUSTOMER APPROVALS</h3>
                  <span className="px-2.5 py-0.5 rounded-full bg-red-500 text-white font-black text-xs animate-bounce">
                    {((stats?.total_pending_approvals || 0) > 0 ? stats.total_pending_approvals : pendingPkgCount)} Action(s) Required
                  </span>
                </div>
                <p className="text-xs text-gray-300 mt-0.5">Review and approve incoming customer service bookings, package purchases, and product store orders.</p>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {/* 1. Service Bookings Card */}
            <Link to="/admin/customer-bookings" className="p-4 bg-white/10 hover:bg-white/15 border border-white/15 rounded-xl transition-all group">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Service Bookings</span>
                <span className={`px-2 py-0.5 text-xs font-bold rounded-md ${stats?.pending_service_bookings > 0 ? 'bg-amber-500 text-slate-950' : 'bg-white/20 text-white'}`}>
                  {stats?.pending_service_bookings || 0} Pending
                </span>
              </div>
              <p className="text-sm font-semibold text-white group-hover:text-red-300 transition-colors flex items-center justify-between">
                <span>Customer Slot Requests</span>
                <ChevronRight className="w-4 h-4 text-white/50 group-hover:translate-x-1 transition-transform" />
              </p>
            </Link>

            {/* 2. Package Approvals Card */}
            <Link to="/admin/package-approvals" className="p-4 bg-white/10 hover:bg-white/15 border border-white/15 rounded-xl transition-all group">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Package Approvals</span>
                <span className={`px-2 py-0.5 text-xs font-bold rounded-md ${(stats?.pending_package_requests || 0) > 0 || pendingPkgCount > 0 ? 'bg-red-500 text-white' : 'bg-white/20 text-white'}`}>
                  {stats?.pending_package_requests || pendingPkgCount || 0} Pending
                </span>
              </div>
              <p className="text-sm font-semibold text-white group-hover:text-red-300 transition-colors flex items-center justify-between">
                <span>Package Purchase Requests</span>
                <ChevronRight className="w-4 h-4 text-white/50 group-hover:translate-x-1 transition-transform" />
              </p>
            </Link>

            {/* 3. Product Store Orders Card */}
            <Link to="/admin/product-orders" className="p-4 bg-white/10 hover:bg-white/15 border border-white/15 rounded-xl transition-all group">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">Product Orders</span>
                <span className={`px-2 py-0.5 text-xs font-bold rounded-md ${stats?.pending_product_orders > 0 ? 'bg-blue-500 text-white' : 'bg-white/20 text-white'}`}>
                  {stats?.pending_product_orders || 0} Pending
                </span>
              </div>
              <p className="text-sm font-semibold text-white group-hover:text-red-300 transition-colors flex items-center justify-between">
                <span>Store Orders & Pickup</span>
                <ChevronRight className="w-4 h-4 text-white/50 group-hover:translate-x-1 transition-transform" />
              </p>
            </Link>
          </div>
